...
def speak(text, key_suffix=""):
    ...
    try:
        ...
        components.html(f"""
        <html><body>
            <style>.speak-btn {{ background: var(--speak-btn-bg); border: 1px solid var(--border-color); border-radius: 12px; padding: 10px; cursor: pointer; display: flex; align-items: center; justify-content: center; width: 100%; font-family: sans-serif; font-size: 14px; color: var(--accent-text-color); transition: 0.2s; }}.speak-btn:hover {{ filter: brightness(0.95); }}.speak-btn:active {{ transform: scale(0.96); }}</style>
