    ...
    st.markdown("""
        <style>
            ...
            .rating-container {
                ...
                border: 3px dashed #333; text-align: center; max-width: 800px; margin-left: auto; margin-right: auto;
            }
            div.stButton > button {
...
def render_sarcastic_sponsor_module():
    ...
    with sponsor_col:
        if st.session_state.taunt_level == 0:
            ...
            st.markdown("""
                <a href="https://p.ecpay.com.tw/" target="_blank" style="display:block; text-align:center; background:#00A650; color:white; padding:15px; border-radius:10px; margin-bottom:10px; font-weight:bold; text-decoration:none; border: 2px solid #000;">💳 綠界贊助 (ECPay)</a>
                <a href="https://www.buymeacoffee.com/" target="_blank" style="display:block; text-align:center; background:#FFDD00; color:black; padding:15px; border-radius:10px; font-weight:bold; text-decoration:none; border: 2px solid #000;">☕ 請我喝咖啡 (Buy Me a Coffee)</a>
            """, unsafe_allow_html=True)
...
def render_game_area(df):
    ...
    for i, bubble in enumerate(bubbles):
        with cols[i]:
            ...
            if st.button(f"👆 戳 {bubble['word']}", key=f"p_{i}", use_container_width=True):
...
def render_bottom_section():
    ...
    <body>
        ...
        <script>function createFloat(el, text) { const f = document.createElement('span'); f.innerText = text; f.className = 'float-text'; el.appendChild(f); setTimeout(() => f.remove(), 1500); }</script>
    </body>
    </html>
    """
    ...
    # 2. 嘲諷 GIF
    st.image("https://media.giphy.com/media/l2JHVUriDGEtWOx0c/giphy.gif", caption="看著你的靈魂...", width=300)
    
    # 3. 搬過來的贊助模組
    render_sarcastic_sponsor_module()
    
    # 4. 頁尾資訊
    st.markdown("<p style='text-align:center; color:#888 !important; font-size:0.8rem;'>v5.6 No Sidebar Edition | 一切都在這裡</p>", unsafe_allow_html=True)
